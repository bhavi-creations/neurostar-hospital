...
class wpbLottier {
	...
	public function enqueue() {
        ...
        /** Front-End Styles */
		wp_register_style( 'mdp-lottier-wpbakery', UnityPlugin::get_url() . 'css/lottier-wpbakery' . UnityPlugin::get_suffix() . '.css', [], UnityPlugin::get_version() );
		wp_enqueue_style( 'mdp-lottier-wpbakery' );

        /** Front-End Scripts */
		wp_enqueue_script( 'lottie-player', UnityPlugin::get_url() . 'js/lottie-player' . UnityPlugin::get_suffix() . '.js', [ 'jquery' ], UnityPlugin::get_version(), true);
		wp_enqueue_script( 'mdp-lottier-wpbakery', UnityPlugin::get_url() . 'js/lottier-wpbakery' . UnityPlugin::get_suffix() . '.js', [ 'lottie-player' ], UnityPlugin::get_version(), true);
    ...
    public function render( $atts ) {
	    ...
	    $link_lottier = explode('json', $this->get_link($settings['animation_url']) );
	    $src =   $link_lottier[0].'json';

	    /** Unique id for current block. */
	    $id = md5( json_encode( $atts ) );

	    /** Enqueue styles only if shortcode used on this page. */
	    wp_enqueue_style( 'mdp-lottier-wpbakery-inline' );
	    ...
	    if ( ! empty( $settings['style_animation_width'] ) ) {
		    $css .= "#mdp-lottier-box-{$id} .mdp-lottier-svg > div { width: {$settings['style_animation_width']}; }";
	    }
	    ...
	    /** Add inline styles. */
	    wp_add_inline_style( 'mdp-lottier-wpbakery-inline', $css );
	    ...
	    <div class="mdp-lottier-svg">
		    ...
		    >
			    ...
			    <?php $css_animation_class = apply_filters( VC_SHORTCODE_CUSTOM_CSS_FILTER_TAG, vc_shortcode_custom_css_class( $settings['style_animation_css'], '' ), 'mdp_wpb_lottier', $atts ); ?>
			    <lottie-player id="mdp-lottier-<?php echo esc_attr($id); ?>" class="mdp-lottier-animation <?php echo esc_attr( $css_animation_class ); ?>" src='<?php echo esc_attr($src); ?>'></lottie-player>
    ...
    public function params_map() {
	    ...
	    if ( true ) {
		    ...
		    $params[] = [
			    ...
			    "type"             => "vc_link",
			    'heading'          => esc_html__( 'Link to json file.', 'lottier-wpbakery' ),
			    ...
			    "description"      => esc_html__( 'Choose animation on ', 'lottier-wpbakery' ).
			                          ...
			                          esc_html__( ' and import it as JSON or URL.', 'lottier-wpbakery' ),
			    'edit_field_class' => 'mdp-control vc_col-xs-12',
		    ];
	    ...
	    if ( true ) {
		    ...
		    $params[] = [
			    ...
			    'group'            => esc_html__( 'Style Animation', 'lottier-wpbakery' ),
			    'value'            => [
				    esc_html__( 'left', 'lottier-wpbakery' )    => 'flex-start',
				    esc_html__( 'center', 'lottier-wpbakery' )  => 'space-around',
				    esc_html__( 'right', 'lottier-wpbakery' )   => 'flex-end',
			    ],
			    'edit_field_class' => 'mdp-control vc_col-xs-12',
		    ];
