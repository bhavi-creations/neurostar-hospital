...
if(!class_exists("Sky_Addons_Pro_Base")) {
	class Sky_Addons_Pro_Base {
    	public $key = "654BCD9FD30C5A8D";
    	private $product_id = "2";
    	private $product_base = "sky-elementor-addons-pro";
    	private $server_host = "https://licenses.techfyd.com/wp-json/licensor/";
		...
		function __construct($plugin_base_file='')
		{
			...
			if($this->hasCheckUpdate) {
				if(function_exists("add_action")){
					add_action( 'admin_post_sky-elementor-addons-pro_fupc', function(){
					...
					if($this->isTheme){
						...
						add_filter( 'plugin_row_meta', function($links, $plugin_file ){
							if ( $plugin_file == plugin_basename( $this->pluginFile ) ) {
								$links[] = " <a class='edit coption' href='" . esc_url( admin_url( 'admin-post.php' ) . '?action=sky-elementor-addons-pro_fupc' ) . "'>Update Check</a>";
							}
							return $links;
						}, 10, 2 );
